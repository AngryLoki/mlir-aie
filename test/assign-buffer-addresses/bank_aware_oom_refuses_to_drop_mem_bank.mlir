...
// "req"'s own bank pin is perfectly satisfiable; bank-aware fails here only
// because f0-f5 (sized to force this outcome: a near-full tile that still
// fits under linear, bank-oblivious packing) do not all fit at once. Under
// the auto scheme that failure used to silently fall back to
// basic-sequential, which has no notion of banks and would have placed "req"
// wherever its bump pointer landed -- a lie no downstream consumer of
// mem_bank (e.g. DMA routing) could detect. mem_bank is a hard constraint, so
// this must be a terminal error instead of a silent fallback.
