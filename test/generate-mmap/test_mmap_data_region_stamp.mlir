//===- test_mmap_data_region_stamp.mlir ------------------------*- MLIR -*-===//
//
...
// (data_origin/data_length on aie.core), not from re-deriving it here.
//
// The layout below is deliberately one where the two answers differ, so the
// test cannot pass by accident. Free space above the 0x400 stack is:
//     [0x400,  0x2000)  = 0x1C00 bytes
//     [0x4000, 0x8000)  = 0x4000 bytes   <-- what largestFreeRun would pick
// The core records the *smaller* gap. A generator that ignored the stamp and
// recomputed would emit ORIGIN = 0x74000, LENGTH = 0x4000; honouring it gives
// ORIGIN = 0x70400, LENGTH = 0x1C00.
//
// test_mmap_data_region_gap.mlir covers the other half of this contract: IR
// with no stamp at all (never through the allocator) still gets a region
// computed here.
...
module @test_mmap_data_region_stamp {
