...
# Pins down the IRON-side contract for Buffer(mem_bank=): the requested L1
# bank must survive resolution and appear as the mem_bank attribute on the
# emitted aie.buffer, so the allocator treats it as the hard constraint the
# Buffer docstring promises.
